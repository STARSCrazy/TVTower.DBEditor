...
namespace TVTower.Xml
{
	...
	public class XmlPersisterV2
	{
		...
		public MovieOldV2 LoadMovieBase( XmlNode xmlNode )
		{
		...
		public List<MovieOldV2> LoadMovie( XmlNode xmlNode )
		{
			var result = new List<MovieOldV2>();
			MovieOldV2 currentMovie = LoadMovieBase( xmlNode );
			...
			foreach ( XmlLinkedNode movieChild in xmlNode.ChildNodes )
			{
				...
				{
					case "episode":
						var episode = LoadMovieBase( movieChild );
		...
		public NewsOldV2 LoadNewsBase( XmlNode xmlNode )
		{
		...
		public List<NewsOldV2> LoadNews( XmlNode xmlNode )
		{
			var result = new List<NewsOldV2>();
			NewsOldV2 currentMovie = LoadNewsBase( xmlNode );
			result.Add( currentMovie );
			...
			foreach ( XmlLinkedNode movieChild in xmlNode.ChildNodes )
			{
				...
				{
					case "episode":
						var episode = LoadNewsBase( movieChild );
						episode.episode = movieChild.GetAttributeInteger( "number" );
						episode.parentID = currentMovie.id;
						result.Add( episode );
						break;
		...
		public AdvertisingOldV2 LoadAd( XmlNode xmlNode )
		{
...

			{
				...
				foreach ( XmlNode xmlMovie in allMovies )
				{
					foreach ( XmlNode childNode in xmlMovie.ChildNodes )
					{
						...
						{
							case "movie":
								movies.AddRange( LoadMovie( childNode ) );
								break;
							default:
...

			{
				...
				foreach ( XmlNode xmlSeries in allSeries )
				{
					foreach ( XmlNode childNode in xmlSeries.ChildNodes )
					{
						...
						{
							case "serie":
								series.AddRange( LoadMovie( childNode ) );
								break;
							default:
				...
				OldV2Converter.Convert( series, database );
			}

			{
				...
				foreach ( XmlNode xmlNews in allNews )
				{
					foreach ( XmlNode childNode in xmlNews.ChildNodes )
					{
						...
						{
							case "news":
								news.AddRange( LoadNews( childNode ) );
								break;
							default:
